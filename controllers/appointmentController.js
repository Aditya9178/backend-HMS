...
//this function will resolve the clerk user id from the request...
...
//to getAppointments of a doctor...
export const getAppointments = async (req, res) => {
    try {
        ...
        const items= await Appointment.find(filter)
          ...
          .skip(skip)
          .limit(limit)
          .populate("doctorId", "name speciality image").lean();
...
export const getAppointmentsByPatient = async (req, res) => {
    try {
        ...
        console.error("Error in getAppointmentsByPaqtient:", err);
        return res.status(500).json({ success: false, message: "Server Error" });
    }
}
...
export const createAppointment = async (req, res) => {
    try {
        ...
        try{
            ...
            console.warn("Error fetching doctor:", e?.message || e);
        }
...
export const confirmPayment = async (req, res) => {
    try {
        ...
        if (!session_id) return res.status(400).json({ success: false, message: "Missing sessionId" });
        ...
        console.error("ConfirmPaymentError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const updateAppointment = async (req, res) => {
    try {
        ...
        const updated = await Appointment.findByIdAndUpdate(id, update, { new: true, runValidators: true }).populate({path: "doctorId", select: "name speciality image"}).lean();

        return res.json({ success: true, appointment: updated });
    }
    catch (err) {
        console.error("UpdatedAppointmentError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const cancelAppointment = async (req, res) => {
    try {
        ...
        console.error("CancelAppointmentError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const getStats = async (req, res) => {
    try {
        ...
        console.error("getStatsError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const getAppointmentsByDoctor = async (req, res) => {
    try {
        ...
        console.error("getAppointmentsByDoctorError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const getRegisteredUserCount = async (req, res) => {
    try {
        ...
        console.error("getRegisteredUserCountError:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
