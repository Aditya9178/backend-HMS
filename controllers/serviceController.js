...
//Helpers Functions....
//so this function converts array like input into a clean array if empty returns the empty array...
...
// This function takes an array of slot strings and normalizes them into a map where the keys are dates (YYYY-MM-DD) and the values are arrays of time strings (e.g., "10:00 AM"). If a slot string doesn't match the expected format, it's placed in an "unspecified" bucket.
...
export async function createService(req, res) {
    try {
        ...
        res.status(201).json({ success: true, data: saved, message: "Service created successfully"
        });
    }
        catch (err) {
...
export async function getServices(req, res) {
    try {
        ...
        return res.status(200).json({ success: true, data: list, message: "Services retrieved successfully" });
    }
    catch (err) {
     console.error("Error while getting service:", err);
...
export async function getServiceById(req, res) {
    try {
        ...
        return res.status(200).json({ success: true, data: service, message: "Service retrieved successfully" });
    }
        catch (err) {
            console.error("GET ServiceById:", err);
        return res.status(500).json({ success: false, message: "Failed to retrieve services" });
        }
}
...
export async function updateService(req, res) {
    try {
        ...
        //to update each file if already present...
          ...
          try {
            ...
            if (up?.secure_url) {
              ...
              if (existing.imagePublicId) {
                //it will remove the old image from cloudinary and replace it with the new one...
                try {
        ...
        console.error("UpdateServiceError:", err);
...
export async function deleteService(req, res) {
    try {
        ...
        return res.status(200).json({ success: true, message: "Service deleted successfully" });
            ...
            console.error("DeleteServiceError:", err);
